...
local auto_repl_enabled_key = "conjure#client#clojure#nrepl#connection#auto_repl#enabled"
local auto_repl_cmd_key = "conjure#client#clojure#nrepl#connection#auto_repl#cmd"
...
function M.configure_conjure()
	...
	-- Keep Conjure's load hook from starting an auto-REPL in plugin preview
	-- windows. setup_autoconnect enables it only after its buffer guards pass.
	vim.g[auto_repl_enabled_key] = false
	vim.g["conjure#client#clojure#nrepl#connection#auto_repl#hidden"] = true
	vim.g[auto_repl_cmd_key] = bb_auto_repl_cmd
...
function M.configure_conjure_output()
	...
	local function show_conjure_split()
		local current_win = vim.api.nvim_get_current_win()
		open_log_window()
	...
	local function show_conjure_float()
		log["close-visible"]()
	...
	hook.override("display-hud", function(opts)
		...
		if M.conjure_output_mode == "float" then
			if default_display_hud then
				return default_display_hud(opts)
			end
			return
	...
	vim.api.nvim_create_user_command("ConjureUseSplit", function()
		M.conjure_output_mode = "split"
		log["close-hud"]()
		show_conjure_split()
	end, {
		desc = "Show Conjure evaluation output in the log split",
		force = true,
	})

	vim.api.nvim_create_user_command("ConjureUseFloat", function()
		M.conjure_output_mode = "float"
		show_conjure_float()
	end, {
		desc = "Show Conjure evaluation output in the floating HUD",
		force = true,
	})

	vim.api.nvim_create_user_command("ConjureToggleOutput", function()
		if M.conjure_output_mode == "float" then
			vim.cmd("ConjureUseSplit")
		else
			vim.cmd("ConjureUseFloat")
		end
	end, {
		desc = "Toggle Conjure evaluation output between split and float",
		force = true,
	})
end
...
local function auto_repl_cmd(root)
	if root then
		if
			vim.fn.filereadable(vim.fs.joinpath(root, "deps.edn")) == 1
			or vim.fn.filereadable(vim.fs.joinpath(root, "project.clj")) == 1
		then
...
local function current_window_floating()
...
local function maybe_autoconnect(bufnr)
	if
		...
		or not real_file_buffer(bufnr)
		or vim.bo[bufnr].filetype ~= "clojure"
		or connect_pending
	then
		return
	end
	...
	local previous_auto_repl_enabled = vim.g[auto_repl_enabled_key]
	vim.g[auto_repl_enabled_key] = true
	vim.b[bufnr][auto_repl_cmd_key] = auto_repl_cmd(root)

	local ok_port, port = pcall(with_root_cwd, root, function()
		auto_repl["upsert-auto-repl-proc"]()
		return state.get()["auto-repl-port"]
	end)
	vim.g[auto_repl_enabled_key] = previous_auto_repl_enabled
...
function M.setup_buffer_mappings()
	local function set_clojure_keymaps(bufnr)
		...
		pcall(vim.keymap.del, "n", "<localleader>lf", { buffer = bufnr })
		vim.keymap.set("n", "<localleader>lf", "<cmd>ConjureToggleOutput<cr>", {
			buffer = bufnr,
			silent = true,
			desc = "Toggle Conjure output mode",
		})
	end

	local group = vim.api.nvim_create_augroup("ClojureBufferMappings", { clear = true })
