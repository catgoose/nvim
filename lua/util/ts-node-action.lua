...
local function is_clojure_wrappable_node(tsnode)
  if not tsnode then
    return false
  end

  local node_type = tsnode:type()
  if clojure_wrappable_nodes[node_type] then
    return true
  end
...
local function get_clojure_wrappable_node()
  ...
  while current do
    seen[#seen + 1] = current:type()
    if is_clojure_wrappable_node(current) then
      return current
    end
...
local function indent_lines(text, prefix)
...
function M.wrap_clojure_form()
  local tsnode = get_clojure_wrappable_node()
  if not tsnode then
