...
local function handle_attribute_value(tsnode)
  ...
  if multiline then
    local line = node_text:gsub("\n", " "):gsub(quote .. "%s+", quote):gsub("%s+" .. quote, quote):gsub("%s+", " ")
    return line, opts
  else
...
M.attribute_name = function(tsnode)
  local sibling = tsnode:next_named_sibling()
  if not sibling then
    return
  end
...
M.tag_name = function(tsnode)
  return handle_tag_name(tsnode)
end

M.handle_attribute = function(tsnode)
